...
class One_And_One_Assistant_Sitetype_Filter {
	...
	/**
	 * Possible const values for $mode
	 */
	...
	/**
	 * One_And_One_Assistant_Sitetype_Filter constructor.
	 *
	 ...
	 */
	function __construct( $config_paths = null, $market = null, $mode = null, $use_transient = true ) {

		if ( is_array( $config_paths ) ) {
			$this->config_paths = $config_paths;
		} else {
			$this->config_paths = array(
				'sitetypes' => One_And_One_Assistant::get_plugin_dir_path() . 'config/sitetypes.json',
				'plugins'   => One_And_One_Assistant::get_plugin_dir_path() . 'config/plugins.json'
			);
		}
		...
		if (! empty( $market ) ) {
			...
			$this->market = One_And_One_Assistant::get_market();
		}
		
		if (! empty( $mode ) ) {
			$this->mode = $mode;
		} else {
			$this->mode = ( oneandone_is_managed() ? self::MODE_MANAGED : self::MODE_FREE );
		}
	...
	public function get_plugins( $sitetype, $include_categories = array() ) {
		...
		foreach ( $config[ 'plugins' ] as $plugin_slug => $plugin_config ) {
			...
			if ( ! array_key_exists( $plugin_slug, $plugins ) ) {
				...
				$is_plugin_available_for_mode =
					( ( $this->mode == 'any' )
					  || empty( $plugin_config[ 'mode' ] )
					  || ( $plugin_config[ 'mode' ] == $this->mode ) );
