...
class One_And_One_Assistant_Config {
	 ...
	 * @return string
	 */
	public static function get( $param, $section = null, $fallback_param = null ) {
		return self::get_instance()->get_param( $param, $section, $fallback_param );
	}
	 ...
	 * @return boolean
	 */
	public static function feature( $param, $fallback_param = null ) {
		return ( bool ) self::get_instance()->get_param( $param, 'features', $fallback_param );
	}
	...
	public static function set_paths( $config_paths = array() ) {
		...
		if ( ! empty( $config_paths ) ) {
			...
			self::$paths = array(
				...
				2 => One_And_One_Assistant::get_plugin_dir_path() . 'config/branding.ini',
	 ...
	 * @return string
	 */
	public function get_param( $param, $section = null, $fallback_param = null ) {
	 ...
	 * Each cookie name must be prefixed with "uialfred-assistant-config-" to work
	 */
	private function retrieve_cookie_params() {
		...
		if ( array_key_exists( 'features', $this->config ) && is_array( $this->config[ 'features' ] ) ) {
			foreach ( $this->config[ 'features' ] as $param => $value ) {
				...
				if ( isset( $_COOKIE[ 'uialfred-assistant-config-' . $param ] ) ) {
					$this->config[ 'features' ][ $param ] = filter_var( $_COOKIE[ 'uialfred-assistant-config-' . $param ], FILTER_SANITIZE_STRING );
				}
			}
		}
	}
