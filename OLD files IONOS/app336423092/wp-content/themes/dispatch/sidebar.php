	...
	<aside <?php hoot_attr( 'sidebar', 'primary' ); ?>>
		...
		elseif ( current_user_can( 'edit_theme_options' ) ) : // If the sidebar has no widgets.
			...
			the_widget(
				'WP_Widget_Text',
				array(
					'title'  => __( 'Example Widget', 'dispatch' ),
					/* Translators: The %s are placeholders for HTML, so the order can't be changed. */
					'text'   => sprintf( __( 'This is an example widget to show how the Primary sidebar looks by default. You can add custom widgets from the %swidgets screen%s in wp-admin.', 'dispatch' ), '<a href="' . esc_url( admin_url( 'widgets.php' ) ) . '">', '</a>' ),
					'filter' => true,
				),
				array(
