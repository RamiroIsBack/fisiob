...
class One_And_One_Assistant_Dashboard {
	...
	/**
	 * One_And_One_Assistant_Handler_Dashboard constructor.
	 */
	public function __construct() {
		
		// Configure AJAX hook for the Dashboard actions
		add_action( 'wp_ajax_ajaxgmarket', array( 'One_And_One_Assistant_Dashboard', 'setup_german_market' ) );
		add_action( 'wp_ajax_ajaxceditor', array( 'One_And_One_Assistant_Dashboard', 'setup_classic_editor' ) );
	...
	public function configure() {
		...
		$this->is_assistant_completed = ( get_option( 'oneandone_assistant_completed' ) == true );
		...
		$blog_feed = One_And_One_Assistant_Config::get( 'blog_feed_{market}', 'links', 'blog_feed_US' );
	...
	public function render_welcome_panel() {
		...
		if ( $this->is_assistant_completed ) {
			...
			One_And_One_Assistant_View::load_template( 'dashboard/welcome-panel-first-run' );
		}
	}
	...
	/**
	 * Render the HTML output for the classic Editor / Gutenberg Editor
	 * help panel for WordPress 5.0
	 */
	public function render_editor_panel() {
		
		if ( ! array_key_exists( 'gutenberg', $this->wp_plugins )
		     && ( ! array_key_exists( 'classic-editor', $this->wp_plugins ) 
		          || ( $this->wp_plugins[ 'classic-editor' ] === false ) )
		     && ! get_option( 'assistant_editor_panel_dismissed', false ) ) {

			One_And_One_Assistant_View::load_template( 'dashboard/editor-info' );
		}
	}
	...
	public function render_german_market_panel() {
		...
		if ( array_key_exists( 'woocommerce', $this->wp_plugins )
		     ...
		     && ! get_option( 'assistant_gmarket_panel_dismissed', false )
		     && One_And_One_Assistant::get_market() == 'DE' ) {
			...
			One_And_One_Assistant_View::load_template( 'dashboard/german-market-info' );
		}
	}
	...
	public function add_dashboard_widgets() {
		...
		if ( $this->custom_blog_feed ) {
			wp_add_dashboard_widget(
				'assistant_blog_feed_widget',
				__( 'Community News', 'uialfred-assistant' ),
			...
			$widget_backup = array( 'assistant_blog_feed_widget' => $normal_dashboard['assistant_blog_feed_widget'] );
			unset( $normal_dashboard['assistant_blog_feed_widget'] );
	...
	public function render_dashboard_blog_feed_widget() {
		...
		$more_url = One_And_One_Assistant_Config::get( 'blog_{market}', 'links', 'blog_US' );
	...
	public static function setup_german_market() {
		...
		// Setup German Market
		$assets_manager = new One_And_One_Assistant_Assets_Manager( 'eshop' );
		...
		update_option( 'assistant_gmarket_panel_dismissed', true );
	...
	public static function setup_classic_editor() {
		...
		$assets_manager = new One_And_One_Assistant_Assets_Manager();
		$success = $assets_manager->setup_single_plugin( 'classic-editor' );

		// Avoid showing that panel again if the user deletes/deactivates the plugin afterwards 
		update_option( 'assistant_editor_panel_dismissed', true );
	...
	public function enqueue_styles() {
		...
		// Add the Dasg
		wp_enqueue_style( '1and1-assistant-dashboard', One_And_One_Assistant::get_css_url( 'dashboard.css' ), array(), One_And_One_Assistant::VERSION );
	}
	...
	public function enqueue_scripts() {
		...
		// Add global script
		wp_enqueue_script( '1and1-wp-dashboard', One_And_One_Assistant::get_js_url( 'dashboard.js' ), array( 'jquery' ), false, true );

		// Configure the AJAX object for the dashboard script
		wp_localize_script( '1and1-wp-dashboard', 'ajax_dashboard_object', array(
...
new One_And_One_Assistant_Dashboard();