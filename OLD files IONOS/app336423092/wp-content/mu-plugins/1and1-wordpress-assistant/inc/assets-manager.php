...
class One_And_One_Assistant_Assets_Manager {
	...
	/**
	 * @var One_And_One_Assistant_Assets_Adapter
	 */
	...
	public function __construct( $site_type = null ) {
		...
		include_once( One_And_One_Assistant::get_inc_dir_path() . 'assets-adapter.php' );

		$this->site_type = $site_type;
		$this->cache_manager = new One_And_One_Assistant_Cache_Manager();
		$this->assets_adapter = new One_And_One_Assistant_Assets_Adapter();
	}
	...
	public function setup_single_plugin( $plugin_slug ) {
		...
		if ( ! $installed ) {
			...
			$installed = One_And_One_Assistant_Installer::install_plugin( $plugin_data );
		}
		...
		if ( $installed ) {
			...
			do_action( 'assistant_plugin_post_install_' . $plugin_slug );
	...
	public function setup_theme( $theme_slug ) {
		...
		if ( ! empty( $theme_slug ) ) {
			...
			if ( ! array_key_exists( $theme_slug, $installed_themes ) ) {
				...
				$installed = One_And_One_Assistant_Installer::install_theme( $themes[ $theme_slug ] );

				// Post actions after installation
				if ( $installed ) {
					do_action( 'assistant_theme_post_install_' . $theme_slug );
				}
			}
			
			// Activate theme
			switch_theme( $theme_slug );
			do_action( 'assistant_theme_post_activate_' . $theme_slug );
		}
	}
	...
	public function install_missing_plugins( $plugin_slugs, $plugin_data ) {
		...
		foreach ( $plugin_slugs as $plugin_slug ) {
			...
			if ( ! in_array( $plugin_slug, $installed_plugin_slugs ) ) {
				$installed = One_And_One_Assistant_Installer::install_plugin( $plugin_data[ $plugin_slug ] );
	...
	public function activate_plugins( $plugin_slugs ) {
		...
		foreach ( $plugin_paths as $plugin_slug => $plugin_path ) {
			...
			try {
				...
				do_action( 'assistant_plugin_post_activate_' . $plugin_slug );
			}
	...
	function create_assistant_home_page() {
		...
		// Page doesn't exist at all and must be created
			...
			$assistant_home_page = wp_insert_post(
				array(
					'post_content'   => One_And_One_Assistant_View::get_template_content(
						'content/wp-default-homepage',
						array( 'url' => admin_url() )
					),
					'post_title'     => sprintf(
						__( "Welcome to %s", 'uialfred-assistant' ),
						home_url()
					),
					'post_status'    => 'publish',
					'post_type'      => 'page',
