...
class One_And_One_Setup_Wizard_Dispatcher {
	...
	public static function load_recommended_themes() {
		...
		if ( isset( $_POST[ 'site_type' ] ) ) {
			...
			load_template( One_And_One_Wizard::get_views_dir_path() . 'parts/site-type-theme-list.php' );
		}
		die;
	}
	...
	public static function install_plugins_and_themes() {
		...
		if ( isset( $_POST['site_type'] ) && isset( $_POST['theme'] ) ) {
			...
			$plugin_manager = new One_And_One_Plugin_Manager( $site_type );
			...
			$plugin_slugs = One_And_One_Sitetype_Filter::get_plugins_slugs( $site_type, 'recommended' );
			...
			self::log( array( 'website_type' => $site_type, 'method' => 'finished', 'theme_selected' => $theme, 'plugins_selected' => $plugins_imploded ) );
	...
	public static function dispatch_wizard_actions() {
		...
		wp_enqueue_style( '1and1-wp-wizard', One_And_One_Wizard::get_css_url( 'wizard.css' ) );
	...
	private static function _choose_appearance() {
		...
		wp_enqueue_script( '1and1-wp-assistant', One_And_One_Wizard::get_js_url( 'assistant.js' ), array( 'jquery' ), false, true );
		...
		load_template( One_And_One_Wizard::get_views_dir_path() . 'setup-wizard-themes-step.php' );
	}
	...
	private static function _activate() {
		...
		load_template( One_And_One_Wizard::get_views_dir_path() . 'setup-wizard-final-step.php' );
	}
