...
class lens_Recent_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'lens_rp', // Base ID
			__('Lens Recent Posts with Thumbnails', 'lens'), // Name
			array( 'description' => __( 'Display your recent posts, with a Thumbnail.', 'lens' ), ) // Args
		);
	}
	...
	if (isset($instance['title'])) :
		...
		<?php
         ...
         ?>
		         ...
		         ?>
		         <div class='rp-thumb'><a href="<?php the_permalink(); ?>"><img alt="<?php the_title() ?>" src="<?php echo get_stylesheet_directory_uri(); ?>/assets/images/placeholder2.jpg"></a></div>
		         <?php
		         endif; ?>	
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Articles', 'lens' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'lens' );
		}
		?>
		<p>
		<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php esc_html_e( 'Title:','lens' ); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		
		<label for="<?php echo $this->get_field_id( 'no_of_posts' ); ?>"><?php esc_html_e( 'No. of Posts:', 'lens' ); ?></label>
...
add_action( 'widgets_init', 'register_lens_widget' );  
function register_lens_widget() {  
    register_widget( 'lens_Recent_Posts' );  
}  
