...
/* Overrides the load textdomain function for the 'dispatch' domain. */
...
/**
 * Overrides the load textdomain functionality when 'dispatch' is the domain in use. The purpose of 
 * this is to allow theme translations to handle the framework's strings. What this function does is 
 * sets the 'dispatch' domain's translations to the theme's. That way, we're not loading multiple 
 ...
 * WordPress core's is_textdomain_loaded(), which will return true after any translation function is run
 * over a text string with the given domain. The purpose of this function is to simply check if the
 * translation files are loaded.
 *
 * This is only used internally by the framework for checking translations.
 *
 * @since 1.0.0
 * @access public
 ...
 * parent theme.
 *
 * Important! Do not use this for translation functions in your theme. Hardcode your textdomain string.
 * Your theme's textdomain should match your theme's folder name.
 *
 * @since 1.0.0
 * @access public
 * @global object $hoot
 ...
 * of the mofile for translations. This allows child themes to have a folder called /languages with
 * translations of their parent theme so that the translations aren't lost on a parent theme upgrade.
 *
 * @since 1.0.0
 * @access public
...
/**
 * Gets the region for the currently viewed page. It strips the language from the locale if needed.
 * Note that not all locales will have a region, so this might actually return the same thing as
 * `hoot_get_language()`.
 *
 * @since 1.0.0
 * @access public
